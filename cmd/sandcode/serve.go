...
Server-wide gates (applied to every launched run):
  ...
  --refine-max-attempts               — cap refine iterations`,
	...
	cmd.Flags().StringVar(&f.addr, "addr", "127.0.0.1:8080", "address to bind (loopback by default; a non-loopback bind requires --api-token)")
	cmd.Flags().StringVar(&f.apiToken, "api-token", "", "bearer token required on all endpoints except /healthz (or set SANDCODE_API_TOKEN). Mandatory for a non-loopback bind")
...
func buildGates(g gateFlags) (*governance.Engine, *budget.Guard, orchestrator.RefineOptions, error) {
	if g.refine && len(g.refineVerify) == 0 {
		return nil, nil, orchestrator.RefineOptions{},
			fmt.Errorf("--refine requires --refine-verify (e.g. --refine-verify go,test,./...)")
	}
...
func runServe(ctx context.Context, f serveFlags) error {
	...
	srv := server.New(server.Options{
		...
		// serve()), and pin client-supplied CWD to the server's own repo.
		AuthToken:       apiToken,
